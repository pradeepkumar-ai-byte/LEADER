...
from .models import Task, TaskCategory, RouteDecision, TaskResult
...
class AutonomousAuditor:
    ...
    async def audit_and_fix(self, target_path: str, max_issues: int = 15):
        ...
        coding_backends = [
            s for s in self.registry.connected()
            if TaskCategory.CODING in s.strengths
        ]
        ...
        console.print(f"[dim]Dispatching audit to {len(auditors)} agents: {', '.join(auditor_names)}[/]")
...
"""
        ...
        for backend in auditors:
            ...
            decision = RouteDecision(primary=backend.id, fallback_chain=[], rationale="Audit broadcast")
...
"""
            ...
            decision = RouteDecision(primary=primary_backend.id, fallback_chain=[], rationale="Auto-fix")

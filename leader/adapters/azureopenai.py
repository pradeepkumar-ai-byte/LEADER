            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(url, headers=headers, json=payload, timeout=aiohttp.ClientTimeout(total=180)) as resp:
                    ...
                    if resp.status == 200:
                        ...
                        return TaskResult(task_id=task.task_id, backend_id="azureopenai", output=msg, success=True, latency_ms=latency)
                    else:
                        return TaskResult(task_id=task.task_id, backend_id="azureopenai", output="", success=False, latency_ms=latency, error=f"HTTP {resp.status}")
        except Exception as exc:
            return TaskResult(task_id=task.task_id, backend_id="azureopenai", output="", success=False, latency_ms=(time.monotonic() - t0) * 1000, error=str(exc))
